...
3. 由主Agent生成当天工作计划
4. 生成“本轮总结反馈”
5. 给仓库创建/更新一条 `night-shift-report` 日报

当前的定向分工规则：

- `data-curator`：主补数据本体。先清民办招生缺口，再清民办学费缺口，最后才补画像。
- `qa-reviewer`：主做门禁和验收。基础字段、重复学校、来源等级、覆盖率趋势都归它盯。
- `main-orchestrator`：根据 QA 指标出日计划，明确今天第一优先级是谁在补、谁在验。
- `policy`：只在招生公告、简章、摇号规则等口径冲突时介入，不直接替代数据维护。
...
- 全天每 2 小时运行一轮
- QA 必须盯基础字段完整度，不只看页面
- QA 结果要汇报给主Agent，由主Agent制定当天计划并分派任务
- 每次跑完都必须有“本轮总结反馈”，不能只有日志
- 当招生缺口和学费缺口并存时，先派 `data-curator` 清招生，再清学费，`qa-reviewer` 同步验收
